//line 149 to 151, CK's pan curves, to avoid 0 when the pan is at max
...
void partial::SetPan( float pan ) // 0.5 = hard right. -0.5 = hard left.
{
	...
	float temp =(pan+0.5f)*1.5707963268f;//CK's change
	pan_right_level = sinf(temp)+0.000001f;//CK's change
	pan_left_level  = cosf(temp)+0.000001f;//CK's change
