...
//I adapted his code for the compatibility with last Jeff's SDK3 files.
//lines 65 to 122, 170
...
struct sampleChannel
{
    ...
    // Loop crossfade length in samples to reduce clicking at loop points
    static constexpr int loopCrossfadeSamples = 32;
	...
	void DoLoop(bool gateState)
	{
		...
		else
		{
            ...
            ptrdiff_t loopLength = s_loop_end - s_loop_st;
            if (loopLength <= 0)
            {
                // Defensive guard to disable loop on invalid lengths
                s_end_section = s_end;
                return;
            }

            // Crossfade region start pointer offset
            ptrdiff_t crossfadeStartOffset = loopLength > loopCrossfadeSamples ? loopLength - loopCrossfadeSamples : 0;
            //ptrdiff_t currentOffset = s_ptr_r - s_loop_st;
             ptrdiff_t currentOffset = s_ptr - s_loop_st;

            // When inside crossfade region, wrap around smoothly (interpolation expected in audio mixing layer)
            if (currentOffset >= crossfadeStartOffset)
            {
                s_ptr = s_loop_st + (currentOffset - loopLength);
            }
            else
            {
                s_ptr = s_loop_st + (currentOffset % loopLength);
            ...
            assert(s_ptr >= sampleStartAddressRight && s_ptr <= s_end);
        }
