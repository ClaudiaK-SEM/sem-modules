...
REGISTER_PLUGIN( KxScopeV4, L"KX SCOPE V4" );
...
KxScopeV4::KxScopeV4( IMpUnknown* host ) : MpBase( host )
,index_( 0 )
//,init( 0 )
{
...
int32_t KxScopeV4::open()
{




	//SET_PROCESS( &KxScopeV4::subProcess );
   ...
   // KxScopeV4 must transmit an initial value on all output pins.
...
void KxScopeV4::waitForTrigger1(int bufferOffset, int sampleFrames)
{
	...
	for(int s = sampleFrames ; s > 0 ; s--)
	{
		...
		{
			index_ = 0;
			SET_PROCESS(&KxScopeV4::waitForTrigger2);
...
void KxScopeV4::waitForTrigger2(int bufferOffset, int sampleFrames)
{
	float* signala	= bufferOffset + pinSignalA.getBuffer();
...
void KxScopeV4::subProcess(int bufferOffset, int sampleFrames)
{
...
void KxScopeV4::subProcessCruise(int bufferOffset, int sampleFrames)
{
	...
	for(int s = sampleFrames ; s > 0 ; s--)
	{
		...
		/*if(timeout_ < 0)
		{
			SET_PROCESS(&KxScopeV4::waitForTrigger1);
			waitForTrigger1(bufferOffset + sampleFrames - s, s);
			return;
		}*/
	}
        //timeout_ -= sampleFrames;
...
void KxScopeV4::forceTrigger()
{
	index_ = 0;
	SET_PROCESS(&KxScopeV4::subProcess);
}

void KxScopeV4::sendResultToGui(int block_offset)
{
/*	int timeR=25;
    ...
    //int min_dsp =(int)(ms*50);//20ms ,2400
    int min_dsp =(int)(getSampleRate()/10.0f);//15
	if(B_SIZE<=min_dsp)timeout_=min_dsp;
	else
	timeout_=B_SIZE;
	SET_PROCESS(&KxScopeV4::subProcessCruise);
}

void KxScopeV4::onSetPins(void)  // one or more pins_ updated.  Check pin update flags to determin which ones.
{


   if(pinOn.isUpdated())
   {
        if(pinOn==1)// && pinSignalA.isStreaming())
        {
        index_ = 0;
        SET_PROCESS(&KxScopeV4::subProcess);
        setSleep(false);
        ...
        //if(pinOn<=0 || (!pinSignalA.isStreaming() && pinSignalA==0.0f))
        {
        pinSamplesA.setValueRaw( 0,0);
        pinSamplesA.sendPinUpdate();
        setSleep(true);
